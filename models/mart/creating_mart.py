...
def model(dbt, session):
    ...
    X = df[[
        "avg_transactions_per_day", "crypto_unlocked", "is_standard_user", "is_premium_user",
        "is_metal_user", "average_amount_per_transaction_usd", "active_timeframe",
        "apple_user", "android_user", "age", "nb_notifications", "direction_ratio"
    ]]
    y = df["active_user"]
    ...
    engagement_features = [
        "avg_transactions_per_day", "crypto_unlocked", "is_standard_user", "is_premium_user",
        "is_metal_user", "average_amount_per_transaction_usd", "active_timeframe", "direction_ratio"
    ]
    scaler = MinMaxScaler()
    ...
    # Segment clustering
    X_kmeans = df_scaled[["LES"]]
    kmeans = KMeans(n_clusters=3, random_state=42)
    df_scaled['segment_kmeans'] = kmeans.fit_predict(X_kmeans)
    cluster_order = df_scaled.groupby('segment_kmeans')['LES'].mean().sort_values().index
    segment_labels = {cluster_order[0]: 'Low Engagement', cluster_order[1]: 'Medium Engagement', cluster_order[2]: 'High Engagement'}
    df_scaled['segment_label'] = df_scaled['segment_kmeans'].map(segment_labels)

    # Merge results for mart
    mart_user_LES_Neo_Bank = df_scaled[["user_id", "LES", "segment_label"]].merge(df, on="user_id", how="left")
    # Churn probability
    X_full_data = df[[
        "avg_transactions_per_day", "crypto_unlocked", "is_standard_user", "is_premium_user",
        "is_metal_user", "average_amount_per_transaction_usd", "active_timeframe", "apple_user",
        "android_user", "age", "nb_notifications", "direction_ratio"
    ]]
    X_full_data_scaled = ML_scaler.transform(X_full_data)
    probabilities = model_lr.predict_proba(X_full_data_scaled)
    churn_probability = probabilities[:, 0]  # probability of churn/inactive
    df_churn_prediction = pd.DataFrame({'user_id': df['user_id'], 'churn_probability': churn_probability})
